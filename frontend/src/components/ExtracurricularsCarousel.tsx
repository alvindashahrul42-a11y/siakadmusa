...
export default function ExtracurricularsCarousel({ extracurriculars, loading, apiBaseUrl }: Props) {
  const [active, setActive] = useState(0);
  const [imageErrors, setImageErrors] = useState<Record<string, boolean>>({});
  const autoplayRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const total = extracurriculars.length;
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(135deg, #2E2D8F 0%, #3D3CAA 50%, #2E2D8F 100%)' }}
    >
      {/* Subtle pattern */}
      <div
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-14">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Ekstrakurikuler</h2>
        ...
        {loading && (
          <div className="text-center py-16">
            ...
            <p className="mt-4" style={{ color: 'rgba(245,197,24,0.8)' }}>Memuat ekstrakurikuler...</p>
          </div>
        )}
        ...
        {!loading && total === 0 && (
          <div className="text-center py-16">
            ...
            <p style={{ color: 'rgba(245,197,24,0.7)' }}>Informasi ekstrakurikuler belum tersedia</p>
          </div>
        )}
                ...
                >
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <span className="inline-flex items-center gap-1.5 mt-3 text-xs font-semibold px-3 py-1 rounded-full" style={{ color: '#2E2D8F', background: '#eef0ff', border: '1px solid #c7c7f0' }}>
                        <span className="w-1.5 h-1.5 rounded-full" style={{ background: '#2E2D8F' }} />
                        Aktif
                      </span>
                    </div>
                  </div>
